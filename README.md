...
.
	...
	gobuster dir -u http://HOST/ -w /usr/share/dirb/wordlists/big.txt -r -k -t 100 -e --no-error -r -o gobuster -a Mozilla/5.0 --exclude-length 123456 -x php,bkp,old,txt,xml,cgi,pdf,html,htm,asp,aspx,pl,sql,js,png,jpg,jpeg,config

.

	ffuf -u https://site.com/FUZZ -w /usr/share/dirb/wordlists/big.txt -mc all -c -t 100 -e .php,.bkp,.old,.txt,.xml,.cgi,.pdf,.html,.htm,.asp,.aspx,.pl,.sql,.js,.png,.jpg,.jpeg,.config -o ffuf-site-raiz
...
Notes: Quando rodar o Check-Setup e aparecer a mensagem "the default postgresql version is not the one used for gvmd" de Rode os comando `sudo pg_lsclusters` e de acordo com a saída dê um upgrade no último `pg_upgradecluster 15` e depois rode e `sudo systemctl restart postgresql` -> `sudo gvm-setup` -> `sudo greenbone-feed-sync` -> `sudo gvm-check-setup` depois pode acessar a web.
